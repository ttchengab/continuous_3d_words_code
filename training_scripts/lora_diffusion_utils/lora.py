...
UNET_DEFAULT_TARGET_REPLACE = {"CrossAttnDownBlock2D", "CrossAttnUpBlock2D", "UNetMidBlock2DCrossAttn", "Attention", "GEGLU"}

UNET_EXTENDED_TARGET_REPLACE = {"ResnetBlock2D", "CrossAttnDownBlock2D", "CrossAttnUpBlock2D", "UNetMidBlock2DCrossAttn", "Attention", "GEGLU"}

TEXT_ENCODER_DEFAULT_TARGET_REPLACE = {"CLIPAttention"}

TEXT_ENCODER_EXTENDED_TARGET_REPLACE = {"CLIPAttention"}

CROSS_ATTENTION_TARGET_REPLACE = {"CrossAttnDownBlock2D", "CrossAttnUpBlock2D", "UNetMidBlock2DCrossAttn"}
...
def _find_modules_v2(
    ...
    for module in model.modules():
        if module.__class__.__name__ in ancestor_class:
            # print("Class Name: ")
            listt.append(module.__class__.__name__)
    print(listt)
...
def inject_trainable_lora(
    ...
    print("Updating cross attention only!")
    count = 0
    for _module, name, _child_module in _find_modules(
        ...
        if loras != None:
            # print("This is the shape differences {} and {}".format(_module._modules[name].lora_up.weight.shape, loras[0].shape))
            if _module._modules[name].lora_up.weight.shape != loras[0].shape:
                count += 1
            # print(loras.pop(0).type(torch.float32))
            _module._modules[name].lora_up.weight = nn.Parameter(loras.pop(0).type(torch.float32))
            _module._modules[name].lora_down.weight = nn.Parameter(loras.pop(0).type(torch.float32))
...
def monkeypatch_or_replace_lora_extended(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if (_child_module.__class__ == nn.Linear) or (
            ...
            # print('hi')
            # print(loras)
            if loras == [] or len(loras[0].shape) != 4:
                continue
            _source = (
                _child_module.conv
